...
# 9. Список последних процессов пользователя (безопасный просмотр)
print_header "Последние 5 ваших процессов"
ps -u "$USER" -o pid,comm,%cpu,%mem --sort=-%cpu | head -6
echo ""

# 10. ASCII-арт в радость
print_header "ASCII-art (просто для веселья)"
echo -e "${GREEN}"
cat << "EOF"
    /\_/\\
   ( o.o )
    > ^ <
   Всем привет от Безвредного Кота!
EOF
echo -e "${NC}"
